...
class RunController:
    ...
    def toggle_pause(self):
        ...
        if app.run_state == RunState.RUNNING:
            ...
            app.ui.set_status("Queued execution pause. Waiting for safe point...")
            app.ui.set_pause_label("Continue")
    ...
    def update_progress(self, fraction, msg=None):
        ...
        if msg == "Paused":
            print("Execution paused. Click Continue to resume.")
            app.after(0, lambda: app.ui.set_status("Paused"))
            app.after(0, lambda: app.ui.set_pause_label("Continue"))
    ...
    def _dispatch(self, mode):
        ...
        if mode == "s1":
            ...
            result = S1Processor(progress_callback=cb, stop_event=stop, pause_event=pause).run(run_processing=True, view=False)
            ...
            result = S2Processor(out_dir=field("s2_out"), preview=False, threshold=threshold, progress_callback=cb, stop_event=stop, pause_event=pause).run(run_processing=True, view=False)
            return result.output_path if not stop.is_set() else None

        elif mode == "fusion":
            print("Executing Data Fusion...")
            out = fuse_flood_outputs(Path(field("s1_tif")), Path(field("s2_tif")), Path(field("out_tif")), progress_callback=cb, stop_event=stop, pause_event=pause)
            ...
            s1_path = None
            if hasS1:
                result = S1Processor(progress_callback=cb, stop_event=stop, pause_event=pause).run(run_processing=True, view=False, entry=s1_entry.to_dict())
            ...
            if hasS2:
                threshold = float(field("threshold")) if field("threshold") else None
                result = S2Processor(out_dir=field("s2_out"), preview=False, threshold=threshold, progress_callback=cb, stop_event=stop, pause_event=pause).run(run_processing=True, view=False, entry=s2_entry.to_dict())
                output_path = getattr(result, "output_path", None)
            ...
            if s1_path and s2_path:
                out = fuse_flood_outputs(s1_path, s2_path, Path(field("out_tif")), progress_callback=cb, stop_event=stop, pause_event=pause)
                return out if not stop.is_set() else None
            elif s1_path:
                return s1_path
            elif s2_path:

...
def test_fuse_flood_bits_sums_binary_masks():
    s1 = np.array([[0, 1], [1, 0]], dtype="uint8")
    s2 = np.array([[0.0, 0.25], [1.5, 2.0]], dtype="float32")

    result = fuse_flood_bits(s1, s2)

    expected = np.array([[0, 2], [2, 1]], dtype="uint8")
    np.testing.assert_array_equal(result, expected)


def test_fuse_flood_outputs_writes_uint8_sum(tmp_path: Path):
    ...
    expected = np.array([[1, 1], [1, 2]], dtype="uint8")
